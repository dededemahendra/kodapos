...
export function CashMovementDialog({
  ...
  async function submit() {
    ...
    if (!Number.isInteger(amt) || amt <= 0) {
      setError(t`Jumlah harus lebih dari nol.`);
      return;
    }
    setSubmitting(true);
    setError(null);
              ...
              {movements.map((m) => (
                <li key={m._id} className="flex justify-between p-2">
                  <span className="text-muted-foreground">
                    {m.direction === 'in' ? <Trans>Masuk</Trans> : <Trans>Keluar</Trans>}
                    {m.note ? ` · ${m.note}` : ''}
                  </span>
                  <span
                    className={`tabular-nums ${m.direction === 'in' ? 'text-emerald-600' : 'text-red-600'}`}
                  >
                    {m.direction === 'in' ? '+' : '−'}
