  ...
  return (
    <dl className="grid grid-cols-2 gap-y-2 text-sm">
      <dt className="text-muted-foreground">
        <Trans>Dibuka oleh</Trans>
      </dt>
      <dd>{shift.cashierName}</dd>
      <dt className="text-muted-foreground">
        <Trans>Dibuka pada</Trans>
      </dt>
      <dd>{opened}</dd>
      {closed && (
        <>
          <dt className="text-muted-foreground">
            <Trans>Ditutup pada</Trans>
          </dt>
          <dd>{closed}</dd>
        </>
      )}
      <dt className="text-muted-foreground">
        <Trans>Modal awal</Trans>
      </dt>
      <dd>{formatIDR(shift.openingFloatIDR)}</dd>
      {shift.cashSalesIDR !== undefined && (
        <>
          <dt className="text-muted-foreground">
            <Trans>Penjualan tunai</Trans>
          </dt>
          <dd>{formatIDR(shift.cashSalesIDR)}</dd>
        </>
      )}
      {shift.cashInIDR !== undefined && shift.cashInIDR > 0 && (
        <>
          <dt className="text-muted-foreground">
            <Trans>Kas masuk</Trans>
          </dt>
          <dd>+{formatIDR(shift.cashInIDR)}</dd>
        </>
      )}
      {shift.cashOutIDR !== undefined && shift.cashOutIDR > 0 && (
        <>
          <dt className="text-muted-foreground">
