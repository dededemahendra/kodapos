...
export function RegisterTopBar() {
  ...
  return (
    <>
      {/* Sits under the bar rather than inside it: every operational screen
          (register, tables, kitchen, incoming orders) shares this chrome, and
          the outage affects all of them. Renders nothing while online. */}
      <header className="sticky top-0 z-50 flex h-12 shrink-0 items-center justify-between gap-2 border-b bg-background px-4">
        <div className="flex items-center gap-2">
          {cafe?.name && <span className="text-sm font-semibold">{cafe.name}</span>}
          <Button asChild size="sm" variant={isActive('/tables') ? 'secondary' : 'ghost'}>
            <Link to="/tables">
          ...
          <Button asChild size="sm" variant={isActive('/self-orders') ? 'secondary' : 'ghost'}>
            <Link to="/self-orders">
          ...
          <Button asChild size="sm" variant={isActive('/kitchen') ? 'secondary' : 'ghost'}>
            <Link to="/kitchen">
          ...
          <Button asChild size="sm" variant={isActive('/history') ? 'secondary' : 'ghost'}>
            <Link to="/history">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span className="hidden lg:inline">
              <Trans>Layar pelanggan</Trans>
            </span>
          </Button>
          <Button asChild size="sm" variant={isActive('/shift') ? 'secondary' : 'ghost'}>
            <Link to="/shift/close">
          ...
          {isOwner && (
            <Button asChild size="sm" variant={isActive('/dashboard') ? 'secondary' : 'ghost'}>
              <Link to="/dashboard">
