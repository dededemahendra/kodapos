...
import { Activity, CreditCard, DoorClosed, DoorOpen } from 'lucide-react';
...
import { formatIDRCompact, formatRelative } from '~/lib/formater';
  ...
  if (item.type === 'sale') {
    return (
      <span>
        ...
        {item.amountIDR != null && (
          <span className="ml-1 text-muted-foreground">{formatIDRCompact(item.amountIDR)}</span>
        )}
      </span>
    );
  }
    ...
    <DashboardCard className="gap-0">
      ...
      <CardContent className="px-0">
        {data === undefined ? (
          ...
          <ul className="flex flex-col divide-y divide-border">
            {data.map((item) => (
              <li className="flex h-16 items-center gap-3 px-6" key={`${item.type}-${item.at}`}>
                <span
                  aria-hidden="true"
                ...
                <div className="min-w-0 flex-1 space-y-1">
                  ...
                  <p className="text-muted-foreground text-xs">{formatRelative(item.at)}</p>
                </div>
              </li>
            ))}
          </ul>
