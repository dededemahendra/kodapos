...
describe('businessOverview', () => {
  it('single-outlet owner: one row whose totals equal that row', async () => {
    ...
    await seedOrder(t, refs, {
      at: wib(2026, 5, 10),
      total: 10000,
      lines: [{ name: 'Kopi', qty: 2, lineTotal: 10000 }],
    });
  ...
  it('sums totals across two outlets and sorts rows by name', async () => {
    ...
    await seedOrder(t, refs, {
      at: wib(2026, 5, 10),
      total: 30000,
      lines: [{ name: 'Kopi', qty: 3, lineTotal: 30000 }],
    });
    ...
    const aShift = await refs.asOwner.mutation(api.shifts.open, {
      cashierId: aCashier,
      openingFloatIDR: 0,
    });
    const aCat = await refs.asOwner.mutation(api.menu.categories.create, { name: 'Teh' });
    const aItem = await refs.asOwner.mutation(api.menu.items.create, {
      categoryId: aCat,
      name: 'Teh',
      priceIDR: 20000,
    });
    await seedOrder(
      t,
      { ...refs, cafeId: alpha, cashierId: aCashier, shiftId: aShift, itemId: aItem },
  ...
  it('returns only the manager accessible outlets', async () => {
    ...
    await seedOrder(t, refs, {
      at: wib(2026, 5, 10),
      total: 99000,
      lines: [{ name: 'Kopi', qty: 1, lineTotal: 99000 }],
    });
    ...
    const gShift = await refs.asOwner.mutation(api.shifts.open, {
      cashierId: gCashier,
      openingFloatIDR: 0,
    });
    const gCat = await refs.asOwner.mutation(api.menu.categories.create, { name: 'X' });
    const gItem = await refs.asOwner.mutation(api.menu.items.create, {
      categoryId: gCat,
      name: 'X',
      priceIDR: 15000,
    });
    await seedOrder(
      t,
      { ...refs, cafeId: granted, cashierId: gCashier, shiftId: gShift, itemId: gItem },
  ...
  it('throws no outlet access for a member with zero granted outlets', async () => {
    ...
    const businessId = (await t.run((ctx) => ctx.db.get(refs.cafeId)))!
      .businessId as Id<'businesses'>;
    // A manager member with NO memberOutletAccess rows -> empty accessible set.
    const mgrUserId = await t.run((ctx) =>
      ctx.db.insert('users', { name: 'Mgr', email: 'noaccess@x.com' })
    );
    await t.run((ctx) =>
      ctx.db.insert('businessMembers', {
        businessId,
    ...
    const asMgr = t.withIdentity({ subject: `${mgrUserId}|test_session` });
    await expect(asMgr.query(api.reports.businessOverview, { range: RANGE })).rejects.toThrow(
      'no outlet access'
    );
  });
});
...
describe('overview is unchanged after the refactor', () => {
  it('still returns the active outlet metrics', async () => {
    ...
    await seedOrder(t, refs, {
      at: wib(2026, 5, 10),
      total: 12000,
      lines: [{ name: 'Kopi', qty: 4, lineTotal: 12000 }],
    });
