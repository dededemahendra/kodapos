...
describe('XenditProvider', () => {
  ...
  it('createCharge throws on non-2xx', async () => {
    ...
    await expect(
      new XenditProvider(CFG).createCharge({ amountIDR: 1000, referenceId: 'x' })
  ...
  it('parseReference extracts data.qr_id', () => {
    const body = JSON.stringify({
      event: 'qr.payment',
      data: { qr_id: 'qr_abc', reference_id: 'ord-1', status: 'SUCCEEDED' },
    });
...
describe('resolveProvider', () => {
  it('returns XenditProvider for a complete xendit config, else MockProvider', () => {
    expect(
      resolveProvider({ provider: 'xendit', secretApiKey: 'k', callbackToken: 't' })
    ).toBeInstanceOf(XenditProvider);
...
describe('XenditProvider.fetchStatus', () => {
  ...
  it('maps a succeeded payment to paid', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(
        JSON.stringify({ id: 'qr_1', status: 'ACTIVE', payments: [{ status: 'SUCCEEDED' }] }),
        { status: 200 }
      )
    );
  ...
  it('maps an active unpaid QR to pending and inactive to expired', async () => {
    vi.spyOn(globalThis, 'fetch')
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ id: 'qr_2', status: 'ACTIVE', payments: [] }), {
          status: 200,
        })
      )
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ id: 'qr_3', status: 'INACTIVE', payments: [] }), {
          status: 200,
        })
      );
