...
New strings introduced in Task 1: `Stok rendah`, `Nilai stok total`. (The low-stock
tile shows a bare count with no unit word, so there is no `bahan` string — and
`Stok rendah` already exists in the catalog from the stock-page filter chip / item
card. Extract will report only what's genuinely new, which is just `Nilai stok total`.)
...
For entries already present with a filled `en` value (`Stok rendah` is already in the
catalog as "Low stock"), leave them as-is. Only fill the empty `msgstr ""` for the
genuinely new entry (`Nilai stok total`).
